						...
						@if (count($actividad->actividadComedor->beneficiariosComedor) != 0)
							@foreach ($actividad->actividadComedor->beneficiariosComedor as $beneficiario)
								<tr>
									...
									<td>
										<a href="{{ action('BeneficiarioController@editBeneficiario',[$actividad->idActividad,  $beneficiario->pivot->idBeneficiarioComedor ]) }}"><button class="btn btn-ff-yellow"><i class="fa fa-eye"></i> Ver Detalles</button></a>
